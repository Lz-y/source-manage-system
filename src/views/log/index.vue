<template>
  <div class="logs-manage-page" v-loading="loading">
    ...
    <CustomTable :columns="columns" :data="list">
      <template #code="{ row }">
        <span :class="row.code === 0 ? 'invalid' : 'available'">{{
          row.code
        }}</span>
      </template>
      <template #status="{ row }">
        <span :class="row.status === 0 ? 'available' : 'invalid'">{{
...
const columns = ref<Array<ColumnProps>>([
  ...
  { attrs: { prop: 'code', label: '请求状态', width: 90 }, _slot: true },
