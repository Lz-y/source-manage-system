  ...
  <div class="create-article-page" v-loading="loading">
    ...
    >
        ...
        <el-col :xs="4" :sm="6" :md="6">
          ...
          >
            <img
              ...
              class="img"
            />
            <el-icon v-else class="img-upload-icon">
              <plus />
            </el-icon>
          </el-upload>
        </el-col>
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="handling" @click="confirm"
          >确认</el-button
        >
      </template>
    </el-dialog>
...
const publishForm$ = ref<InstanceType<typeof Query> | null>(null)
...
function switchEncrypt(state: boolean) {
  if (!state) {
    ...
    nextTick(() => {
      publishForm$.value?.$el.querySelector('input[type=password]').focus()
    })
  } else {
