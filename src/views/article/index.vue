<template>
  <div class="article-manage-page" v-loading="loading">
    <Query :configs="configs" :data="formData" inline>
      <el-button type="primary" :icon="Search" @click="getAllArticle"
        >查询</el-button
      >
    ...
    <CustomTable :columns="columns" :data="tableData" stripe>
      ...
      <template #tags="{ row }">
        <div class="tags">
          <el-tag v-for="tag in row.tags" size="small" :key="'tag-' + tag">{{
            tag
          }}</el-tag>
        </div>
      </template>
      <template #encrypt="{ row }">
        <span :class="row.encrypt === 1 ? 'available' : 'encrypted'">{{
          EncryptStatusOptions[row.encrypt].label
        }}</span>
      </template>
      <template #status="{ row }">
        <span :class="row.status === 0 ? 'no-reply' : 'available'">{{
      ...
      <template #operation="{ row }">
        <el-button
          type="primary"
          size="small"
          @click="showEncryptDislog(row)"
          text
          >{{ row.encrypt === 0 ? '公开' : '加密' }}</el-button
        >
        <el-popconfirm
        ...
        <el-button type="primary" size="small" @click="jumpTo(row)" text
          >编辑</el-button
        >
      </template>
    </CustomTable>
...
const columns = ref<Array<ColumnProps>>([
  ...
  { attrs: { prop: 'classify', label: '类别', width: 60 } },
...
const encryptForm$ = ref<InstanceType<typeof Query> | null>(null)
...
function showEncryptDislog(row: Article) {
  ...
  nextTick(() => {
    encryptForm$.value?.$el.querySelector('input[type=password]').focus()
  })
}
function close() {
  nextTick(() => {
...
async function confirm() {
  try {
    ...
    await modifyArticle(
      encryptForm.id,
      { psw: encryptForm.psw!, encrypt: encryptForm.encrypt! },
      `${encryptForm.encrypt! === 0 ? '加密' : '公开'}成功`
    )
  } catch (error) {
    throw error
  } finally {
...
async function getAllArticle() {
  ...
  try {
    ...
    const params: Partial<
      Pick<Article, 'title' | 'classify' | 'tags' | 'encrypt' | 'status'>
    > = { title, classify, encrypt, status }
...
async function getDicts() {
  try {
    const [tags, encryptStatus, articleStatus] = await Promise.all([
      getOneDict({ type: 'SYS_TAGS' }),
      getOneDict({ type: 'SYS_ENCRYPT_STATUS' }),
      getOneDict({ type: 'SYS_POST_STATUS' }),
    ])
